...
const techFolders = [
  ...
  },
  {
    ...
    techs: [
      ...
      { id: "coolify", icon: "skill-icons:docker", title: "Coolify" },
  ...
  },
  {
    ...
    techs: [
      ...
      { id: "loki", icon: "devicon:grafana", title: "Loki" },
      { id: "sentry", icon: "material-icon-theme:sentry", title: "Sentry" },
      { id: "bugsink", icon: "simple-icons:bugsnag", title: "Bugsink" }
    ]
  },
  {
...
export default function TechStackFolders() {
  ...
  useEffect(() => {
    const updateLines = () => {
      ...
      const folders = containerRef.current.querySelectorAll(
        "[data-folder-index]"
      )
      const paths = containerRef.current.querySelectorAll("path")

      folders.forEach((folderEl, i) => {
        if (i >= folders.length - 1) return
        const folder = folderEl
        const nextFolder = folders[i + 1]
        if (!nextFolder) return

        const rect1 = folder.getBoundingClientRect()
        const rect2 = nextFolder.getBoundingClientRect()
        const containerRect = containerRef.current.getBoundingClientRect()

        // Start: bottom center of current folder
        const x1 = rect1.left + rect1.width / 2 - containerRect.left
        const y1 = rect1.bottom - containerRect.top

        // End: top center of next folder (slightly offset for visual flow)
        const x2 = rect2.left + rect2.width / 2 - containerRect.left
        const y2 = rect2.top + 40 - containerRect.top
    ...
    >
      <div className="max-w-7xl mx-auto w-full">
        ...
        <div ref={containerRef} className="relative group">
          {/* SVG Overlay for Curved Connecting Lines */}
          <svg className="absolute inset-0 w-full h-full pointer-events-none z-3 overflow-visible">
            <defs>
            ...
            {techFolders.map((_, index) => (
              index < techFolders.length - 1 && (
                <path
                  key={index}
                  d=""
                  ...
                  strokeLinecap="round"
                  fill="none"
                  className="opacity-0 transition-opacity duration-1500 ease-out"
                />
              )
            ))}
          </svg>

          {/* Staggered Grid of Folders */}
    ...
    <>
      ...
      >
        <div
          ...
          style={{
            background:
              "radial-gradient(circle at 50% 70%, var(--primary) 0%, transparent 70%)"
          }}
        />
        ...
        >
          {/* Back layer */}
          <div
            className="absolute w-40 h-32 bg-gradient-to-b from-muted to-muted/80 rounded-3xl shadow-xl"
            style={{
          ...
          {/* Tab */}
          <div
            className="absolute w-16 h-6 bg-primary/20 rounded-3xl backdrop-blur-sm border-t border-primary/30"
            style={{
              top: "22px",
              left: "56px",
              transform: isHovered ? "rotateX(-30deg)" : "rotateX(0deg)",
              transition: "transform 600ms cubic-bezier(0.34, 1.56, 0.64, 1)",
              zIndex: 11
            }}
          />
          ...
          {/* Front layer */}
          <div
            className="absolute w-40 h-32 bg-card border border-border rounded-3xl shadow-2xl"
            style={{
              top: "38px",
              transform: isHovered
                ? "rotateX(28deg) translateY(14px)"
                : "rotateX(0deg)",
              transition: "transform 600ms cubic-bezier(0.34, 1.56, 0.64, 1)",
              zIndex: 30
            }}
        ...
        <h3 className="text-xl font-bold text-foreground mt-4 tracking-wide">
          {title}
        </h3>
        <p className="text-sm text-muted-foreground mt-1">
          {projects.length} technologies
        </p>
      </div>
...
// ====================== TechCard ======================
const TechCard = forwardRef(
  ({ icon, title, delay, isVisible, index, onClick, isSelected }, ref) => {
...
export function TechLightbox({
  ...
  const navigateNext = () =>
    internalIndex < projects.length - 1 && onNavigate(internalIndex + 1)
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          transform: animationPhase === "complete" ? "scale(1)" : "scale(0.5)",
      ...
      >
        <div className="bg-card/95 backdrop-blur-2xl rounded-3xl shadow-2xl border border-border/50 overflow-hidden">
          ...
          {internalIndex > 0 && (
            <button
              ...
              className="absolute left-8 top-1/2 -translate-y-1/2 w-14 h-14 rounded-full bg-muted/80 backdrop-blur flex items-center justify-center hover:bg-muted"
            >
          ...
          {internalIndex < projects.length - 1 && (
            <button
              ...
              className="absolute right-8 top-1/2 -translate-y-1/2 w-14 h-14 rounded-full bg-muted/80 backdrop-blur flex items-center justify-center hover:bg-muted"
            >
        ...
        <button
          onClick={handleClose}
          className="absolute top-8 right-8 w-12 h-12 rounded-full bg-muted/80 backdrop-blur flex items-center justify-center hover:bg-muted"
        >
          <X className="w-6 h-6" />
        </button>
      </div>
