...
export default function FanCarousel({ items, renderCard, onCenterChange, initialCenter }) {
  ...
  const needsPagination = totalItems > MAX_VISIBLE;

  const getInitialCenter = () => {
    if (initialCenter !== undefined && initialCenter >= 0 && initialCenter < totalItems) {
      return needsPagination ? initialCenter : totalItems >> 1;
    }
    return needsPagination ? HALF : totalItems >> 1;
  };

  const [centerIndex, setCenterIndex] = useState(getInitialCenter);
  ...
  const getVisibleMap = useCallback((center) => {
    ...
    for (let slot = 0; slot < MAX_VISIBLE; slot++) {
      map.set(((center + slot - HALF) % totalItems + totalItems) % totalItems, slot);
    }
    return map;
  }, [totalItems, needsPagination]);
  ...
  const cycle = useCallback((direction) => {
    ...
    setCenterIndex(prev =>
      direction === "right" ? (prev + 1) % totalItems : (prev - 1 + totalItems) % totalItems
    );
  }, [totalItems, needsPagination]);

  useEffect(() => {
    if (onCenterChange) onCenterChange(centerIndex);
  ...
  useEffect(() => {
    ...
    const slotCount = needsPagination ? MAX_VISIBLE : totalItems;
      ...
      visibleEntries.forEach(({ el, slot }) => {
        ...
        if (hoveredSlot !== null) {
          ...
          if (slot === hoveredSlot) {
            ...
            const normalized = (slot - Math.floor(visibleEntries.length / 2)) / (Math.floor(visibleEntries.length / 2) || 1);
            ...
            if (slot === visibleEntries.length - 1 && hoveredSlot < Math.floor(visibleEntries.length / 2)) targetY -= 1 * hM;
            if (slot === 0 && hoveredSlot > Math.floor(visibleEntries.length / 2)) targetY -= 1 * hM;
          }
        } else {
          delay = Math.abs(slot - Math.floor(visibleEntries.length / 2)) * 0.02;
        }
  ...
  }, [centerIndex, totalItems, getVisibleMap, needsPagination]);
          ...
          {items.map((item, index) => {
            const cardContent = renderCard ? (
              ...
              <div className="relative w-full h-full overflow-hidden">
                {item.imgUrl && (
                  <img src={item.imgUrl} loading="lazy" alt={item.alt || `Card ${index}`} className="absolute inset-0 w-full h-full object-cover z-10" />
                )}
              </div>
            );

            return item.linkUrl ? (
              <a key={item.id || index} href={item.linkUrl} target={item.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className="fan-card block cursor-pointer flex-shrink-0">{cardContent}</a>
            ) : (
              <div key={item.id || index} className="fan-card flex-shrink-0">{cardContent}</div>
            );
          })}
        </div>
      </div>
